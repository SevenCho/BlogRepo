//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

@interface MidasIAPCommonUtility : NSObject
{
}

+ (id)getValueFromString:(id)arg1 byKey:(id)arg2;
+ (BOOL)isProductStillNeedReprovide:(id)arg1;
+ (id)midasUserDefaultsForKey:(id)arg1;
+ (void)updateMidasUserDefaults:(id)arg1 value:(id)arg2;
+ (id)localPriceFromLocale:(id)arg1 price:(id)arg2;
+ (void)registerBugly;
+ (void)loadClassMannually;
+ (struct CGSize)getScreenSizeByOrientation;
+ (BOOL)isIpValid:(id)arg1;
+ (id)gunzippedData:(id)arg1;
+ (id)gzippedData:(id)arg1;
+ (BOOL)isGzippedData:(id)arg1;
+ (BOOL)isSystemVersionAboveiOS7;
+ (void)registerUserAgent:(id)arg1;
+ (id)bundleSeedID:(int *)arg1;
+ (id)createGUID;
+ (BOOL)checkGUIDService;
+ (void)findService:(id)arg1 tag:(id)arg2 saveUID:(id)arg3 group:(id)arg4;
+ (void)registerAppUid;
+ (void)registerUid;
+ (void)registerInitVid;
+ (id)getMacAddress;
+ (BOOL)isWechatInstalled;
+ (BOOL)isQQInstalled;
+ (BOOL)isDeviceJailBroken;
+ (void)clearBackgroundMask;
+ (void)setBackgroundMask:(id)arg1 alpha:(float)arg2 orientation:(int)arg3;
+ (id)getSubImage:(id)arg1 orientation:(int)arg2;
+ (id)getScreenShotsImage:(int)arg1;
+ (void)showAlert:(id)arg1 msg:(id)arg2 cancelBtnTitle:(id)arg3 completion:(CDUnknownBlockType)arg4 onViewController:(id)arg5;
+ (struct CGSize)getSizeRatio:(int)arg1;
+ (BOOL)ignoreOrientation;
+ (id)md5:(id)arg1;
+ (float)rotateAngleFromOrientation:(int)arg1 toOrientation:(int)arg2;
+ (struct CGAffineTransform)transformForOrientation:(int)arg1;
+ (id)filePath:(id)arg1;
+ (id)vendorString;
+ (id)platform;
+ (id)imageNamed:(id)arg1;
+ (struct CGSize)getSystemScreenMetrics:(int)arg1;
+ (id)getSubview:(id)arg1 byTagId:(unsigned int)arg2;
+ (void)load;

@end

