//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

#import "NSCoding.h"

@class NSString;

@interface AccountBindPhoneInfo : NSObject <NSCoding>
{
    NSString *m_nsTitle;
    NSString *m_nsDetail;
    unsigned long m_uiSafeRemindType;
    unsigned long m_uiMobileCanBeFoundShowPos;
    BOOL m_bForce;
}

@property(nonatomic) BOOL m_bForce; // @synthesize m_bForce;
@property(nonatomic) unsigned long m_uiMobileCanBeFoundShowPos; // @synthesize m_uiMobileCanBeFoundShowPos;
@property(nonatomic) unsigned long m_uiSafeRemindType; // @synthesize m_uiSafeRemindType;
@property(retain, nonatomic) NSString *m_nsDetail; // @synthesize m_nsDetail;
@property(retain, nonatomic) NSString *m_nsTitle; // @synthesize m_nsTitle;
- (void).cxx_destruct;
- (BOOL)isMobileCanBeFoundShowInFillPhoneViewController;
- (id)getHeadIcon;
- (void)ParseFromStyle:(id)arg1;
- (void)ParseFromXml:(id)arg1;
- (id)init;
- (id)initWithCoder:(id)arg1;
- (void)encodeWithCoder:(id)arg1;

@end

