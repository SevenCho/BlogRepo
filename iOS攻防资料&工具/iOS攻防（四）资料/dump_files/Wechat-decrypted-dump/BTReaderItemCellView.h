//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "BTBaseItemCellView.h"

#import "WCActionSheetDelegate.h"

@class BTReaderItemCellViewModel, CALayer, MMUILabel, MMWebImageView, NSString, RichTextView;

@interface BTReaderItemCellView : BTBaseItemCellView <WCActionSheetDelegate>
{
    RichTextView *m_titleLabel;
    RichTextView *m_digestLabel;
    MMWebImageView *m_coverImageView;
    CALayer *m_itemHighlightLayer;
    CALayer *m_separateLayer;
    MMUILabel *m_friendReadLabel;
    BOOL m_bIsForceHighlight;
    unsigned long _itemIndex;
}

@property(nonatomic) unsigned long itemIndex; // @synthesize itemIndex=_itemIndex;
- (void).cxx_destruct;
- (id)undoneMenuItem;
- (id)debugMenuItem;
- (id)deleteMenuItem;
- (id)readLaterMenuItem;
- (id)favoriteMenuItem;
- (id)forwardMenuItem;
- (struct CGRect)previewingSourceRectForLocation:(struct CGPoint)arg1 inCoordinateView:(id)arg2;
- (id)viewControllerToPreviewWithFromController:(id)arg1 forLocation:(struct CGPoint)arg2 inCoordinateView:(id)arg3;
- (BOOL)canPeek;
- (void)actionSheet:(id)arg1 clickedButtonAtIndex:(int)arg2;
- (void)showComfirmDeleteActionSheet;
- (void)onShowUnFinished:(id)arg1;
- (void)onDebug:(id)arg1;
- (void)onDelete:(id)arg1;
- (void)onReadLater:(id)arg1;
- (void)onFavorite:(id)arg1;
- (void)onForward:(id)arg1;
- (void)handleMenuControllerWillShowNotification:(id)arg1;
- (void)handleMenuControllerWillHideMenuNotification:(id)arg1;
- (BOOL)canPerformAction:(SEL)arg1 withSender:(id)arg2;
- (BOOL)canBecomeFirstResponder;
- (void)onBrandItemLongPress;
- (void)handleLongPressEx:(id)arg1;
- (void)layoutSubviewsNormalItem;
- (void)layoutSubviewsTopItem;
- (void)layoutSubviews;
@property(readonly, nonatomic) MMUILabel *friendReadLabel;
@property(readonly, nonatomic) RichTextView *digestLabel;
@property(readonly, nonatomic) RichTextView *titleLabel;
@property(readonly, nonatomic) MMWebImageView *coverImageView;
- (void)updateFriendsRead;
- (void)onUpdateViewModel;
- (void)initFriendsReadIconLabel;
- (void)initDigestLabel;
- (void)initTitleLabel;
- (void)initCoverView;
- (void)initSeparateLayer;
- (void)initNormalReaderItem;
- (void)initTopReaderItem;
- (void)initLongPressGesture;
- (void)forceSetHighlighted:(BOOL)arg1;
- (void)setHighlighted:(BOOL)arg1 animated:(BOOL)arg2;
- (void)initItemHighlightLayer;
- (void)setViewModel:(id)arg1;
- (id)initWithViewModel:(id)arg1;
- (void)dealloc;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned int hash;
@property(readonly) Class superclass;
@property(readonly, nonatomic) BTReaderItemCellViewModel *viewModel; // @dynamic viewModel;

@end

