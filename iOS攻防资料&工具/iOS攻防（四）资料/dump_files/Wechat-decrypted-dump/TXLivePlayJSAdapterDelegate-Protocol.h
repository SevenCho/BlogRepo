//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

@class NSDictionary, NSString;

@protocol TXLivePlayJSAdapterDelegate <NSObject>

@optional
- (void)enableFullScreen:(BOOL)arg1 orientation:(int)arg2 playerId:(unsigned long)arg3;
- (void)onInsertLogView:(unsigned long)arg1;
- (void)onPlayNetStatus:(NSDictionary *)arg1 playerId:(unsigned long)arg2;
- (void)onPlayEvent:(int)arg1 evtMsg:(NSString *)arg2 playerId:(unsigned long)arg3 param:(NSDictionary *)arg4;
@end

