//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

@class NSString, WNParagraphParser;

@interface WNParagraphMgr : NSObject
{
    WNParagraphParser *m_parser;
    BOOL bHtmlDownloaded;
    NSString *originHtmlStr;
}

@property(nonatomic) BOOL bHtmlDownloaded; // @synthesize bHtmlDownloaded;
@property(retain, nonatomic) NSString *originHtmlStr; // @synthesize originHtmlStr;
- (void).cxx_destruct;
- (id)getVoiceFmtOfString:(unsigned long)arg1;
- (unsigned long)getVoiceFmt:(id)arg1;
- (void)cutLongTextParagraph:(id)arg1;
- (id)cutSingleLongTextParagraph:(id)arg1;
- (void)tryRemoveLastEmptyParagraph:(id)arg1;
- (void)tagLastTextParagraph:(id)arg1;
- (BOOL)loadHtmlPath:(id)arg1 toParagraph:(id)arg2;
- (BOOL)parseParagraph:(id)arg1 FromVideoData:(id)arg2;
- (BOOL)parseParagraph:(id)arg1 FromFileData:(id)arg2;
- (BOOL)parseParagraph:(id)arg1 FromImageData:(id)arg2;
- (BOOL)parseParagraph:(id)arg1 FromVoiceData:(id)arg2;
- (BOOL)parseParagraph:(id)arg1 FromLocationData:(id)arg2;
- (BOOL)parseParagraph:(id)arg1 FromTextData:(id)arg2;
- (void)parseFavItemFromDataList:(id)arg1 toParagraphs:(id)arg2;
- (BOOL)parseFavItem:(id)arg1 FromHtml:(id)arg2 toParagraphs:(id)arg3;
- (void)exportParagraph:(id)arg1 toFileFavData:(id)arg2;
- (void)exportParagraph:(id)arg1 toVideoFavData:(id)arg2;
- (void)exportParagraph:(id)arg1 toImageFavData:(id)arg2;
- (void)exportParagraph:(id)arg1 toVoiceFavData:(id)arg2;
- (void)exportParagraph:(id)arg1 toLocationFavData:(id)arg2;
- (void)exportParagraph:(id)arg1 toTextFavData:(id)arg2;
- (void)exportParagraphs:(id)arg1 toHtml:(id *)arg2 toDataList:(id)arg3;
- (id)exportParagraphsToHtml:(id)arg1;
- (id)exportTextInfoHtml:(id)arg1;
- (id)getMergeHtmlParagraphs:(id)arg1;
- (id)getMergeDatalistParagraphs:(id)arg1;
- (id)genArticleDesc:(id)arg1;
- (void)parseFavItem:(id)arg1 toParagraphs:(id)arg2 originMessage:(id)arg3 Editable:(BOOL)arg4;
- (void)dealloc;
- (id)init;

@end

