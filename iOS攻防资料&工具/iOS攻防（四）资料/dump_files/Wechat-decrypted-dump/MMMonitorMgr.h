//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

#import "FPSTracerDelegate.h"
#import "IBaseMsgContentViewControllerScrollExt.h"
#import "IMainFrameScrollExt.h"
#import "ITimeLineViewScrollExt.h"
#import "MMUIViewControllerExt.h"

@class NSObject<OS_dispatch_queue>, NSString, NSThread, WXGFPSTracer, WXGMainThreadHandler;

@interface MMMonitorMgr : NSObject <FPSTracerDelegate, MMUIViewControllerExt, ITimeLineViewScrollExt, IMainFrameScrollExt, IBaseMsgContentViewControllerScrollExt>
{
    NSThread *m_monitorThread;
    BOOL m_bStop;
    unsigned int m_nIntervalTime;
    unsigned int m_nLastTimeInterval;
    struct vector<unsigned int, std::__1::allocator<unsigned int>> m_vecLastMainThreadCallStack;
    unsigned int m_nSum;
    BOOL m_isBackgroundStartUp;
    unsigned long long m_blockDiffTime;
    NSString *m_potenHandledLagFile;
    float m_cpuUsage;
    unsigned int m_firstSleepTime;
    struct __CFRunLoopObserver *m_runLoopBeginObserver;
    struct __CFRunLoopObserver *m_runLoopEndObserver;
    struct __CFRunLoopObserver *m_initializationBeginRunloopObserver;
    struct __CFRunLoopObserver *m_initializationEndRunloopObserver;
    BOOL m_bDebugInXcode;
    WXGMainThreadHandler *m_pointMainThreadHandler;
    WXGFPSTracer *m_fpsTracer;
    NSObject<OS_dispatch_queue> *m_ftsDumpQueue;
    unsigned int m_mainSemaphore;
    NSString *m_trackScene;
    BOOL m_bFirstActive;
    BOOL m_bInSuspend;
    BOOL m_bMonitor;
    BOOL m_bLaunchLagMonitorOver;
}

+ (id)shareInstance;
@property(nonatomic) BOOL m_bLaunchLagMonitorOver; // @synthesize m_bLaunchLagMonitorOver;
@property(nonatomic) BOOL m_bMonitor; // @synthesize m_bMonitor;
- (id).cxx_construct;
- (void).cxx_destruct;
- (void)onMainFrameEndScroll;
- (void)onMainFrameBeginScroll;
- (void)onMsgContentViewControllerEndScroll;
- (void)onMsgContentViewControllerBeginScroll:(id)arg1;
- (void)onTimeLineEndScroll;
- (void)onTimeLineBeginScroll;
- (void)MMUIViewControllerDidDisappear:(id)arg1;
- (void)MMUIViewControllerWillDisppear:(id)arg1;
- (void)MMUIViewControllerDidAppear:(id)arg1;
- (void)MMUIViewControllerWillAppear:(id)arg1;
- (void)handleFrameDropTooMuchWithScene:(id)arg1;
- (void)onFrameDroppedTooMuch;
- (id)getLaunchBlockRecordFilePath;
- (void)clearDumpInBackgroundLaunch;
- (void)setLaunchOverByTimer;
- (void)reportWithID:(int)arg1 key:(int)arg2 value:(int)arg3;
- (unsigned long long)diffTime:(struct timeval *)arg1 endTime:(struct timeval *)arg2;
- (id)dumpFileWithType:(unsigned int)arg1;
- (void)saveBlockTimeToLagFile;
- (void)resetStatus;
- (void)setPerStackInterval:(unsigned int)arg1;
- (void)setCPUUsagePercent:(float)arg1;
- (void)setRunloopTimeOut:(unsigned long long)arg1 andCheckPeriodTime:(unsigned int)arg2;
- (void)refreshMonitorSetting;
- (BOOL)needFilter;
- (unsigned int)check;
- (void)threadProc;
- (BOOL)isDumpTypeRelatedToFOOM:(unsigned int)arg1;
- (void)addMonitorThread;
- (void)removeRunLoopObserver;
- (void)addRunLoopObserver;
- (void)willResignActive;
- (void)didEnterBackground;
- (void)didBecomeActive;
- (void)willSuspend;
- (void)willTerminate;
- (void)handleBackgroundLaunch;
- (void)stop;
- (void)dealloc;
- (id)init;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned int hash;
@property(readonly) Class superclass;

@end

